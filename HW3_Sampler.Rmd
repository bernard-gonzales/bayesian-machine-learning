...
# Find all 3-combinations of these predictors
combinations <- combn(predictors, 3, simplify = FALSE)

# Store results
results <- data.frame(variables = character(), overall_accuracy = numeric(), a_accuracy = numeric())

for (combo in combinations) {
  # Fit model with current combination of predictors
  formula <- as.formula(paste("a ~", paste(combo, collapse = " + ")))
  fit <- brm(formula = formula, family = bernoulli(), data = wine, chains = 4, iter = 10, seed = 123)
  
  # Predict and calculate accuracies
  predicted_probs <- fitted(fit, type = "response")
  predicted_classes <- ifelse(predicted_probs > 0.5, 1, 0)
  
  overall_accuracy <- mean(predicted_classes == wine$a)
  a_accuracy <- mean(predicted_classes[wine$a == 1] == wine$a[wine$a == 1])
...
# Find the combinations with the highest accuracies
best_overall <- results[which.max(results$overall_accuracy), ]
best_a <- results[which.max(results$a_accuracy), ]
